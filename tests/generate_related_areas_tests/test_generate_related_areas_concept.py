...
class GenerateRelatedAreasConceptTests(TestCase):
	...
	def test_08_save(self):
		...
		related_area_nodegroup = function_instance.config["relatedarea_name_output_nodegroup"]
		...
		for area_name, returned_area_type in expected_saved_locations.items():
			...
			mapped_type_label = function_instance.mapRelatedAreaTypeLabel(returned_area_type)
