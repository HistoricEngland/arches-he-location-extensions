define(['knockout',
    ...
    'viewmodels/function',
    'bindings/chosen',
    'templates/views/components/functions/generate_related_area_from_map_function.htm'],
    function (ko, _, koMapping, ListView, FunctionViewModel, chosen, GenerateRelatedAreaFromMapFunctionTemplate) {
        return ko.components.register('views/components/functions/generate_related_area_from_map_function', {
                ...
                try {
                    ...
                    this.relatedareatype_output_node.subscribe(function (o_n) {
                        _.each(self.nodesRelatedAreaType(), function (node) {
                            if (node.datatype !== "semantic") {
                                if (o_n === node.nodeid) {
                                    params.config.relatedarea_type_output_nodegroup = node.nodegroup_id;
                                }
                            }
                        });
                    });
            ...
            template: GenerateRelatedAreaFromMapFunctionTemplate
        });
    });